...
exports.deploy1820 = async(signer) => {
    ...
    const code = await signer.provider.getCode(REGISTRY_ADDRESS);

    if (code.toString() === "0x") {
        await signer.sendTransaction({ to: SIGNATURE_ADDRESS, value: COST });
        await signer.provider.sendTransaction(TxERC1820RegistryDeployment);
    }
    return new ethers.Contract(REGISTRY_ADDRESS, ERC1820RegistryAbi, signer);
};

const TxERC1820RegistryDeployment =
